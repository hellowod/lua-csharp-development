...
namespace CsharpLua
{
    internal static class LuaBaseLib
    {
        ...
        public static int B_PCall(ILuaState lua)
        {
            ...
            ThreadStatus status = lua.PCallK(lua.GetTop() - 2, LuaDef.LUA_MULTRET, 0, 0, DG_PCallContinuation);
        ...
        public static int B_XPCall(ILuaState lua)
        {
            ...
            lua.Replace(2);
            ThreadStatus status = lua.PCallK(n - 2, LuaDef.LUA_MULTRET, 1, 0, DG_PCallContinuation);
            return FinishPCall(lua, status == ThreadStatus.LUA_OK);
        }
        ...
        public static int B_RawLen(ILuaState lua)
        {
            ...
            lua.L_ArgCheck(t == LuaType.LUA_TTABLE || t == LuaType.LUA_TSTRING, 1, "table or string expected");
        ...
        {
            int n = lua.GetTop();
            if (lua.Type(1) == LuaType.LUA_TSTRING && lua.ToString(1)[0] == '#') {
                lua.PushInteger(n - 1);
                return 1;
            } else {
        ...
        public static int B_ToNumber(ILuaState lua)
        {
            ...
            if (t == LuaType.LUA_TNONE || t == LuaType.LUA_TNIL) // standard conversion
            {
                ...
                s = s + '\0'; // guard
                int pos = 0;
                if (s[pos] == '-') { pos++; negative = true; } else if (s[pos] == '+') pos++;
