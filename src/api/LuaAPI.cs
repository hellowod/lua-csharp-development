    ...
    public partial class LuaState : ILuaState
    {
        ...
        private static void F_Load(ref LoadParameter param)
        {
            ...
            if (c == LuaConf.LUA_SIGNATURE[0]) {
                ...
                proto = LuaUndump.LoadBinary(L, param.LoadInfo, param.Name);
            } else {
                L.CheckMode(param.Mode, "text");
                proto = LuaParser.Parse(L, param.LoadInfo, param.Name);
            }
        ...
        private CallInfo FindPCall()
        {
            for (int i = CI.Index; i >= 0; --i) {
                var ci = BaseCI[i];
                if ((ci.CallStatus & CallStatus.CIST_YPCALL) != 0) { return ci; }
            }
        ...
        void ILuaAPI.SetTop(int index)
        {
            if (index >= 0) {
                ...
                for (int i = Top.Index; i < newTop; ++i) { Stack[i].V.SetNilValue(); }
                Top = Stack[newTop];
            } else {
        ...
        void ILuaAPI.Remove(int index)
        {
            ...
            for (int i = addr.Index + 1; i < Top.Index; ++i) { Stack[i - 1].V.SetObj(ref Stack[i].V); }
        ...
        void ILuaAPI.XMove(ILuaState to, int n)
        {
            ...
            Top = Stack[index];
            for (int i = 0; i < n; ++i) { StkId.inc(ref toLua.Top).V.SetObj(ref Stack[index + i].V); }
        }
        ...
        bool ILuaAPI.CheckStack(int size)
        {
            ...
            if (StackLast - Top.Index > size) { res = true; }
            // need to grow stack
            else {
        ...
        void ILuaAPI.CreateTable(int narray, int nrec)
        {
            ...
            Top.V.SetHValue(tbl);
            ApiIncrTop();
            if (narray > 0 || nrec > 0) { tbl.Resize(narray, nrec); }
        }

        void ILuaAPI.CreateTable()
        {
            API.CreateTable(0, 0);
        }
