...
#pragma warning restore 0429
        ...
        {
            Value = (Value & MASK0(SIZE_OP, POS_OP)) | ((((uint)op) << POS_OP) & MASK1(SIZE_OP, POS_OP));
            return this;
        }
        ...
        {
            Value = ((Value & MASK0(size, pos)) | (((uint)value << pos) & MASK1(size, pos)));
            return this;
        }
    ...
    public static class LuaCoder
    {
        ...
        private static void CodeArith(FuncState fs, OpCode op, ExpDesc e1, ExpDesc e2, int line)
        {
            ...
            int o2 = (op != OpCode.OP_UNM && op != OpCode.OP_LEN) ? Exp2RK(fs, e2) : 0;
            int o1 = Exp2RK(fs, e1);
            if (o1 > o2) {
                FreeExp(fs, e1);
                FreeExp(fs, e2);
        ...
        private static bool PatchTestReg(FuncState fs, int node, int reg)
        {
            ...
            if (reg != NO_REG && reg != pi.Value.GETARG_B()) {
                ...
                pi.Value = Instruction.CreateABC(OpCode.OP_TEST, pi.Value.GETARG_B(), 0, pi.Value.GETARG_C());
            }
            return true;
        }
        ...
        private static void InvertJump(FuncState fs, ExpDesc e)
        {
            ...
            LuaUtil.Assert(TestTMode(pc.Value.GET_OPCODE()) && pc.Value.GET_OPCODE() != OpCode.OP_TESTSET && pc.Value.GET_OPCODE() != OpCode.OP_TEST);
        ...
        private static int JumpOnCond(FuncState fs, ExpDesc e, bool cond)
        {
            if (e.Kind == ExpKind.VRELOCABLE) {
                ...
                if (ie.GET_OPCODE() == OpCode.OP_NOT) {
                    ...
                    return CondJump(fs, OpCode.OP_TEST, ie.GETARG_B(), 0, (cond ? 0 : 1));
                }
                // else go through
            }
            Discharge2AnyReg(fs, e);
            FreeExp(fs, e);
            return CondJump(fs, OpCode.OP_TESTSET, NO_REG, e.Info,  (cond ? 1 : 0));
        }
        ...
        public static void PatchClose(FuncState fs, int list, int level)
        {
            ...
            while (list != NO_JUMP) {
                ...
                LuaUtil.Assert(pi.Value.GET_OPCODE() == OpCode.OP_JMP &&  (pi.Value.GETARG_A() == 0 || pi.Value.GETARG_A() >= level));
        ...
        private static void Discharge2Reg(FuncState fs, ExpDesc e, int reg)
        {
            ...
            switch (e.Kind) {
                ...
                default: {
                        LuaUtil.Assert(e.Kind == ExpKind.VVOID || e.Kind == ExpKind.VJMP);
            ...
            switch (e.Kind) {
                ...
                case ExpKind.VNIL: {
                        ...
                        if (fs.Proto.K.Count <= Instruction.MAXINDEXRK) {
                            e.Info = (e.Kind == ExpKind.VNIL) ? NilK(fs) : BoolK(fs, (e.Kind == ExpKind.VTRUE));
                            e.Kind = ExpKind.VK;
        ...
        public static void Self(FuncState fs, ExpDesc e, ExpDesc key)
        {
            ...
            e.Kind = ExpKind.VNONRELOC;
            ReserveRegs(fs, 2);
            CodeABC(fs, OpCode.OP_SELF, e.Info, ereg, LuaCoder.Exp2RK(fs, key));
            FreeExp(fs, key);
        }
        ...
        {
            var mode = OpCodeInfo.GetMode(op);
            LuaUtil.Assert(mode.OpMode == OpMode.iABx || mode.OpMode == OpMode.iAsBx);
